...
MAC_FIND = raw_input("Введите mac (aaaa.bbbb.cccc): ")    
...
ssh.send('cisco' + '\n')
time.sleep(0.5)
ssh.send('show mac address-table address ' + MAC_FIND + '\n')
time.sleep(1)
mac_address_output = ssh.recv(5000)
mac_address_output_list = mac_address_output.split()
mac_address_output_list.remove(MAC_FIND)
interface = mac_address_output_list[mac_address_output_list.index(MAC_FIND)+2]
...
for ip_switch in switches_to_connect:
    if switch_ip_mac_vendor[ip_switch][1][0:6] == 'Allied':
        ...
        if type_of_telesis_switch == 'at8000':
            find_mac_on_telesis_switch_8000(ip_switch, 'manager', 'friend', MAC_FIND)
        elif type_of_telesis_switch == 'fs970':
            find_mac_on_telesis_switch_970(ip_switch, 'manager', 'friend', MAC_FIND)
    else:
        if switch_ip_mac_vendor[ip_switch][1][0:5] == 'Cisco':
            ...
            if type_of_cisco_switch == 'c2900':
                find_mac_on_cisco_switch(ip_switch, LOGIN_CORE, PASSWORD_CORE, MAC_FIND)
            elif type_of_cisco_switch == 'sf300':
                find_mac_on_cisco_switch_sf300(ip_switch, LOGIN_CORE, PASSWORD_CORE, MAC_FIND)
        else:
