...
def find_mac_on_cisco_switch(ip,username,password,mac):
    ...
    if int(cisco_number_mac_addresses) < 20:
        print ('Device found on cisco switch ' + ip)
    ...
    try:
        ...
        if interface[0:2] != 'ch':
            ...
            if len(telesis_number_mac_addresses) < 12:
                print ('Device found on Allied telesis switch ' + ip)
                print('Interface is  ' + interface)
            telesis.close()
        else:
            telesis.close()
...
IP = raw_input("Enter IP of device:  ")
IP_CORE = raw_input("Enter core's switch IP: ")
LOGIN_CORE = raw_input("Enter core's switch username: ")
PASSWORD_CORE = getpass.getpass("Enter core's switch password: ")
sucess_login = 0
while sucess_login != 1:
    try:
...
for ip_switch in switches_to_connect:
    if switch_ip_mac_vendor[ip_switch][1][0:6] == 'Allied':
        ...
        print ('Finding on ' + type_of_telesis_switch + ' ' + ip_switch)
        ...
        if switch_ip_mac_vendor[ip_switch][1][0:5] == 'Cisco':
            ...
            print ('Finding on ' + type_of_cisco_switch + ' ' + ip_switch)
            ...
            print ('Vendor not found, coud not connect to it, sorry.' + ip_switch)
